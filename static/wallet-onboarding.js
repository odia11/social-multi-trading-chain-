...
function modal(){
  ...
  m.addEventListener('click',function(e){if(e.target===m)close();var b=e.target.closest('[data-action]');if(!b)return;var a=b.dataset.action;if(a==='create')showCreate();if(a==='import')showImport();if(a==='phantom'){close();bypass=true;var x=document.getElementById('oa-guest-connect-btn');if(x)x.click();setTimeout(function(){bypass=false},50);}});
  return m;
}
...
document.addEventListener('click',function(e){var b=e.target&&e.target.closest?e.target.closest('#oa-guest-connect-btn'):null;if(!b||bypass)return;e.preventDefault();e.stopPropagation();e.stopImmediatePropagation();open();},true);
