...
ensureCommonLoader();
function svg(type){var p={reply:'<path d="M21 15a4 4 0 0 1-4 4H8l-5 3 1.5-4.5A8 8 0 1 1 21 15Z"/>',repost:'<path d="M17 3l4 4-4 4"/><path d="M3 7h18"/><path d="M7 21l-4-4 4-4"/><path d="M21 17H3"/>',like:'<path d="M20.8 4.6a5.4 5.4 0 0 0-7.6 0L12 5.8l-1.2-1.2a5.4 5.4 0 0 0-7.6 7.6L12 21l8.8-8.8a5.4 5.4 0 0 0 0-7.6Z"/>'};return '<svg class="oa-feed-action-icon" viewBox="0 0 24 24" aria-hidden="true">'+p[type]+'</svg>'}
function classify(btn,index){var s=((btn.className||'')+' '+(btn.getAttribute('aria-label')||'')+' '+(btn.getAttribute('title')||'')+' '+(btn.getAttribute('onclick')||'')).toLowerCase();if(s.indexOf('emoji')>-1||s.indexOf('react')>-1||s.indexOf('share')>-1||s.indexOf('bookmark')>-1||s.indexOf('save')>-1)return null;if(s.indexOf('like')>-1||s.indexOf('heart')>-1)return'like';if(s.indexOf('repost')>-1||s.indexOf('retweet')>-1)return'repost';if(s.indexOf('reply')>-1||s.indexOf('comment')>-1)return'reply';return null}
function upgrade(row){var btns=Array.prototype.slice.call(row.querySelectorAll('.fc-action'));btns.forEach(function(btn,i){if(btn.dataset.oaIcon==='1')return;var type=classify(btn,i);if(!type)return;var oldIcon=btn.querySelector('svg,img,.fc-action-icon');if(oldIcon)oldIcon.style.display='none';if(type==='like'){var legacyHeart=btn.querySelector('.fc-heart-ico');if(legacyHeart)legacyHeart.style.display='none'}btn.classList.add('oa-action-'+type);btn.insertAdjacentHTML('afterbegin',svg(type));btn.dataset.oaIcon='1'});}
