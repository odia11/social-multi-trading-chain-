...
(function(){
  ...
  var css=document.createElement('link');css.rel='stylesheet';css.href='/static/live-market-redesign.css?v=4';document.head.appendChild(css);
...
document.addEventListener('DOMContentLoaded',function(){
  ...
  if(searchIn)searchIn.addEventListener('input',function(){var q=searchIn.value.trim();clearTimeout(_searchTimer);if(q.length<2){if(searchRes)searchRes.classList.remove('open');return}_searchTimer=setTimeout(function(){runSearch(q)},300)});
  function runSearch(q){Promise.allSettled([fetch('/api/dexscreener/search?q='+encodeURIComponent(q)).then(function(r){return r.json()}),fetch('/api/users/search?q='+encodeURIComponent(q),{credentials:'include'}).then(function(r){return r.json()})]).then(function(results){var tokRes=results[0].status==='fulfilled'?results[0].value:null,userRes=results[1].status==='fulfilled'?results[1].value:null,pairs=((tokRes&&tokRes.pairs)||[]).filter(function(p){return _NB_LIVE_CHAINS.indexOf(p.chainId)!==-1}).slice(0,6),users=(userRes&&userRes.ok&&userRes.users)||[],html='';if(pairs.length)html+='<div class="pt-nb-sr-hd">Tokens</div>'+pairs.map(function(p){var sym=p.baseToken.symbol,addr=p.baseToken.address,img=p.info&&p.info.imageUrl,chainLbl=_NB_CHAIN_LABELS[p.chainId]||p.chainId;return'<div class="pt-nb-sr-row" data-action="tok" data-mint="'+esc(addr)+'" data-chain="'+esc(p.chainId)+'">'+logoTile(img,sym,'pt-nb-sr-logo','pt-nb-sr-logo-ph')+'<div class="pt-nb-sr-name">$'+esc(sym)+' <span class="pt-nb-sr-chain">'+esc(chainLbl)+'</span></div><div class="pt-nb-sr-sub">'+fmtPrice(p.priceUsd)+'</div></div>'}).join('');if(users.length)html+='<div class="pt-nb-sr-hd">Traders</div>'+users.slice(0,5).map(function(u){return'<div class="pt-nb-sr-row" data-action="trader" data-wallet="'+esc(u.wallet)+'">'+logoTile(u.avatar_url,u.username,'pt-nb-sr-logo','pt-nb-sr-logo-ph')+'<div class="pt-nb-sr-name">'+esc(u.username||'')+'</div></div>'}).join('');if(searchRes){searchRes.innerHTML=html||'<div class="pt-nb-sr-empty">No results</div>';searchRes.classList.add('open')}})}
