...
function chainLabel(c){c=String(c||'').toLowerCase();return {solana:'SOL',bsc:'BNB',base:'BASE',arbitrum:'ARB',polygon:'POLYGON',robinhood:'ROBINHOOD'}[c]||String(c||'CHAIN').toUpperCase()}
function shortPair(s){s=String(s||'');return s.length>14?s.slice(0,7)+'…'+s.slice(-5):s||'—'}
function pctChip(label,v){var n=num(v),cl=n!=null&&n<0?' neg':'';return '<div class="oa-fc-pct'+cl+'"><span>'+esc(label)+'</span><b>'+esc(fmtPct(v))+'</b></div>'}
...
function tokenRoute(c){
  var mint=String(c&&c.mint||'').trim();
  var pair=String(c&&c.pairAddress||'').trim();
  var sym=String(c&&c.symbol||'').trim();
  if(mint)return '/live-market?mint='+encodeURIComponent(mint);
  if(pair)return '/live-market?pair='+encodeURIComponent(pair);
  if(sym)return '/live-market?q='+encodeURIComponent(sym);
  return '/live-market';
}
function goToken(e,card){
  if(e){e.preventDefault();e.stopPropagation();}
  var d=parseData(card)||{};
  window.location.href=tokenRoute(d);
}
function render(card,c){
  ...
  card.innerHTML=''
    ...
    +'<div class="oa-fc-flow"><div class="oa-fc-flow-left"><div class="oa-fc-flow-label">Buys / Sells</div><div class="oa-fc-flow-values"><span class="oa-fc-buys" data-cc="buys">'+buys+'</span><span> / </span><span class="oa-fc-sells" data-cc="sells">'+sells+'</span></div></div><div class="oa-fc-flow-right"><div><span class="oa-fc-buys">'+Math.round(bpct)+'%</span> / <span class="oa-fc-sells">'+Math.round(100-bpct)+'%</span></div><div class="oa-fc-bar"><span data-cc="buysbar" style="width:'+bpct.toFixed(1)+'%"></span></div></div></div>'
    +'<div class="oa-fc-pcts">'+pctChip('5m',c.chg5m)+pctChip('1h',c.chg1h)+pctChip('6h',c.chg6h)+pctChip('24h',c.chg24h)+'</div>'
    +'<div class="oa-fc-bottom"><div class="oa-fc-pair"><span>PAIR</span><code>'+esc(shortPair(pair))+'</code></div><button type="button" class="oa-fc-cta">View Token <span>›</span></button></div></div>';
  var cta=card.querySelector('.oa-fc-cta');if(cta)cta.addEventListener('click',function(e){goToken(e,card)});
