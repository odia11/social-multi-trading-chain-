...
trade:'<path d="M5 7h13"/><path d="m15 4 3 3-3 3"/><path d="M19 17H6"/><path d="m9 14-3 3 3 3"/>',
...
function build(){if(document.getElementById('oa-bottom-nav'))return;var nav=document.createElement('nav');nav.id='oa-bottom-nav';nav.className='oa-bottom-nav';nav.setAttribute('aria-label','Mobile navigation');var p=here(),wallet=p==='/wallet';nav.innerHTML='<a href="/" class="'+(p==='/'?'active':'')+'">'+icon('home')+'<span class="oa-nav-label">Home</span></a>'+
'<a href="/live-market" class="'+(p==='/live-market'?'active':'')+'">'+icon('market')+'<span class="oa-nav-label">Live Market</span></a>'+
(wallet?'<button type="button" class="oa-trade-main oa-wallet-main active" aria-label="Portfolio actions">'+icon('walletAction')+'<span>Actions</span></button>':'<a href="/live-market" class="oa-trade-main '+(p==='/live-market'?'active':'')+'" aria-label="Trade">'+icon('trade')+'<span>Trade</span></a>')+
...
document.body.appendChild(nav);var btn=nav.querySelector('.oa-menu-btn');if(btn)btn.addEventListener('click',function(){var existing=document.getElementById('pt-nb-menu-btn');if(existing){existing.click();return}var more=document.getElementById('pt-nb-more-btn');if(more)more.click()});var walletBtn=nav.querySelector('.oa-wallet-main');if(walletBtn)walletBtn.addEventListener('click',openWalletSheet);buildWalletSheet()}
