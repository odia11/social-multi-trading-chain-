/* OrcAgent Live Market critical controller — one real gold chart + mobile search. */
(function(){
'use strict';
...
function injectCss(){
  ...
  s.textContent=[
    ...
    '.oa-critical-chart{position:absolute!important;left:0!important;right:0!important;top:38px!important;bottom:24px!important;width:100%!important;height:calc(100% - 62px)!important;display:block!important;opacity:1!important;visibility:visible!important;z-index:10!important;touch-action:pan-y pinch-zoom!important}',
    '.oa-critical-status{position:absolute!important;inset:52px 16px 34px!important;z-index:9!important;display:flex;align-items:center;justify-content:center;text-align:center;color:#657180;font:700 11px/1.4 "JetBrains Mono",monospace;pointer-events:none}',
...
function installSearch(){
  ...
  function open(){
    ...
    requestAnimationFrame(function(){try{input.focus({preventScroll:true});}catch(_){input.focus();}});
  }
  function close(){
    if(!layer) return;
  ...
  document.addEventListener('click',function(e){if(e.target.closest('#pt-nb-search-close'))setTimeout(close,0);},true);
  document.addEventListener('keydown',function(e){if(e.key==='Escape')setTimeout(close,0);},true);
}
...
function showStatus(st,text){
  st.status.textContent=text||'';
  st.status.style.display=text?'flex':'none';
}
function draw(st){
  ...
  if(mn===mx){mn*=.999;mx=mx*1.001||1;}
  var pad=Math.max((mx-mn)*.10,Math.abs(cur)*.00025);mn-=pad;mx+=pad;
  ...
  var picks=[0,Math.floor((st.data.length-1)*.25),Math.floor((st.data.length-1)*.5),Math.floor((st.data.length-1)*.75),st.data.length-1];
  var seenIdx={},seenText={};
  picks.forEach(function(j){
    if(seenIdx[j]) return;seenIdx[j]=1;
    var d=new Date(Number(st.data[j].t)*1000);
    var t=('0'+d.getHours()).slice(-2)+':'+('0'+d.getMinutes()).slice(-2);
    if(seenText[t]) return;seenText[t]=1;
    var anchor=j===0?'start':j===st.data.length-1?'end':'middle';
    html+='<text x="'+X(j)+'" y="'+(h-6)+'" fill="#718091" font-size="9" text-anchor="'+anchor+'" font-family="monospace">'+t+'</text>';
  });
...
function normalizeCandles(a){
  ...
  var out=a.map(function(c){
    return {t:Number(c.t),o:Number(c.o!=null?c.o:c.c),h:Number(c.h!=null?c.h:c.c),l:Number(c.l!=null?c.l:c.c),c:Number(c.c),v:Number(c.v)||0};
  }).filter(function(c){return c.t>0&&c.c>0&&isFinite(c.c);}).sort(function(x,y){return x.t-y.t;});
  var dedup=[];
  out.forEach(function(c){if(dedup.length&&dedup[dedup.length-1].t===c.t)dedup[dedup.length-1]=c;else dedup.push(c);});
  return dedup;
}
function hasRealHistory(a){
  if(!a||a.length<3) return false;
  var times={},mn=Infinity,mx=-Infinity;
  a.forEach(function(c){times[c.t]=1;mn=Math.min(mn,c.l||c.c,c.c);mx=Math.max(mx,c.h||c.c,c.c);});
  if(Object.keys(times).length<3) return false;
  if(!(mn>0)||!(mx>0)) return false;
  return (mx-mn)/Math.max(mx,mn)>0.000001;
}
...
function fetchBackend(st){
  return fetch(backendUrl(st),{credentials:'include',cache:'no-store'}).then(function(r){return r.ok?r.json():null;}).then(function(j){
    if(!j) return null;
    if(j.pair_address&&!st.pair) st.pair=j.pair_address;
    var a=normalizeCandles(j.candles);
    return {candles:a,current:Number(j.current_price)||0,fallback:j.fallback===true,source:'backend'};
...
function fetchGecko(st){
  ...
  return fetch(u,{cache:'no-store',headers:{Accept:'application/json;version=20230203'}}).then(function(r){return r.ok?r.json():null;}).then(function(j){
    var rows=j&&j.data&&j.data.attributes&&j.data.attributes.ohlcv_list;
    if(!Array.isArray(rows)) return null;
    var a=normalizeCandles(rows.map(function(x){return {t:x[0],o:x[1],h:x[2],l:x[3],c:x[4],v:x[5]};}));
    return {candles:a,current:a.length?Number(a[a.length-1].c)||0:0,fallback:false,source:'gecko'};
  }).catch(function(){return null;});
}
function applyData(st,res){
  if(!res||res.fallback||!hasRealHistory(res.candles)) return false;
...
function load(st){
  ...
  st.loading=true;showStatus(st,'Loading real chart history…');
  Promise.all([fetchBackend(st),fetchGecko(st)]).then(function(all){
    var back=all[0],gecko=all[1];
    if(applyData(st,gecko)) return true;
    if(applyData(st,back)) return true;
    st.data=[];st.pts=null;st.svg.innerHTML='';
    showStatus(st,'Waiting for real market history…');
    return false;
...
function tick(st,p){
  ...
  if(!st.data||!st.data.length){
    st.liveBuffer=st.liveBuffer||[];
    var now=Math.floor(Date.now()/1000);
    if(!st.liveBuffer.length||st.liveBuffer[st.liveBuffer.length-1].c!==p) st.liveBuffer.push({t:now,o:p,h:p,l:p,c:p,v:0});
    if(st.liveBuffer.length>60) st.liveBuffer=st.liveBuffer.slice(-60);
    if(st.liveBuffer.length>=3&&hasRealHistory(st.liveBuffer)){st.data=st.liveBuffer.slice();draw(st);}
    return;
  }
  var cfg=tfCfg(st.tf),now2=Math.floor(Date.now()/1000),bucket=Math.floor(now2/cfg.sec)*cfg.sec;
  ...
  else{last.c=p;last.h=Math.max(Number(last.h||p),p);last.l=Math.min(Number(last.l||p),p);}
...
function animate(st,next){
  ...
  function frame(now){
    ...
    if(q<1) st.anim=requestAnimationFrame(frame);else{st.anim=null;tick(st,next);}
  }
...
function bind(st){
  var active=false,intent='',startX=0,startY=0,raf=null,pending=0;
  function run(x){pending=x;if(raf)return;raf=requestAnimationFrame(function(){raf=null;scrub(st,pending);});}
  st.svg.addEventListener('pointerdown',function(e){
    if(e.pointerType==='mouse'){active=true;intent='scrub';run(e.clientX);return;}
    startX=e.clientX;startY=e.clientY;intent='';active=false;
  });
  st.svg.addEventListener('pointermove',function(e){
    if(e.pointerType==='mouse'){run(e.clientX);return;}
    var dx=e.clientX-startX,dy=e.clientY-startY;
    if(!intent){
      if(Math.max(Math.abs(dx),Math.abs(dy))<7)return;
      intent=Math.abs(dx)>Math.abs(dy)*1.15?'scrub':'scroll';
      if(intent==='scrub'){
        active=true;
        try{st.svg.setPointerCapture(e.pointerId);}catch(_){}
      }
    }
  ...
  function end(){active=false;intent='';if(raf){cancelAnimationFrame(raf);raf=null;}clearScrub(st);}
...
function setup(card){
  var wrap=card.querySelector('.pt-chart-wrap'),m=getMint(card);if(!wrap||!m)return;
  var old=states.get(card);if(old&&old.svg&&old.svg.isConnected)return;
  wrap.querySelectorAll('.oa-critical-chart,.oa-critical-tip,.oa-critical-vline,.oa-critical-dot,.oa-critical-status').forEach(function(n){n.remove();});
  var svg=document.createElementNS('http://www.w3.org/2000/svg','svg');svg.classList.add('oa-critical-chart');svg.setAttribute('preserveAspectRatio','none');
  ...
  var status=document.createElement('div');status.className='oa-critical-status';status.textContent='Loading real chart history…';
  wrap.appendChild(svg);wrap.appendChild(status);wrap.appendChild(line);wrap.appendChild(dot);wrap.appendChild(tip);
  var activeBtn=card.querySelector('.pt-tf-pill.active');
  var st={id:Math.random().toString(36).slice(2),card:card,wrap:wrap,svg:svg,tip:tip,line:line,dot:dot,status:status,mint:m,pair:getPair(card),chain:getChain(card),tf:(activeBtn&&activeBtn.dataset.tf)||'5m',data:[],liveBuffer:[],current:domPrice(card),loading:false,anim:null,pts:null,geom:null};
  states.set(card,st);bind(st);load(st);
}
function scan(){
  document.querySelectorAll('.pt-card').forEach(setup);
  states.forEach(function(st,card){if(!card.isConnected){if(st.anim)cancelAnimationFrame(st.anim);states.delete(card);}});
}
function handleTf(e){
  ...
  st.data=[];st.liveBuffer=[];st.svg.innerHTML='';clearScrub(st);load(st);
}
function prices(){
  if(document.hidden)return;
  var groups={};
  states.forEach(function(st){if(st.card.isConnected&&st.pair)(groups[st.chain]=groups[st.chain]||[]).push(st);});
  Object.keys(groups).forEach(function(ch){
    ...
    fetch('/api/market/prices?chain='+encodeURIComponent(ch)+'&pairs='+encodeURIComponent(pairs.join(',')),{credentials:'include',cache:'no-store'})
      ...
      .then(function(j){if(!j||!j.prices)return;arr.forEach(function(st){var p=Number(j.prices[(st.pair||'').toLowerCase()]);if(p>0)animate(st,p);});})
      .catch(function(){});
  });
}
function boot(){
