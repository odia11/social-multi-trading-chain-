...
_MAX_IDENTITY_SCAN_NODES = 2000
_MAX_IDENTITY_SCAN_DEPTH = 24
...
def _contains_forbidden_identity(obj, *, _state=None, _depth=0) -> bool:
    # Scan the whole structured payload rather than only top-level keys. Nested
    # objects are a common way to bypass authorization shims when a downstream
    # handler later reads fields such as order.user_id or wallet.address.
    if _state is None:
        _state = [0]
    _state[0] += 1
    if _state[0] > _MAX_IDENTITY_SCAN_NODES or _depth > _MAX_IDENTITY_SCAN_DEPTH:
        raise ValueError('Financial request payload is too complex')
    if isinstance(obj, dict):
        for key, value in obj.items():
            if str(key).strip().lower() in _FORBIDDEN_IDENTITY_KEYS:
                return True
            if _contains_forbidden_identity(value, _state=_state, _depth=_depth + 1):
                return True
...
def _request_tries_identity_override() -> bool:
    if {str(k).strip().lower() for k in request.args.keys()} & _FORBIDDEN_IDENTITY_KEYS:
        return True
    if request.is_json:
        body = request.get_json(silent=True)
        if body is not None and _contains_forbidden_identity(body):
            return True
    return False
...
def install(appmod) -> None:
    ...
    def _financial_authorization_guard():
        ...
        try:
            override = _request_tries_identity_override()
