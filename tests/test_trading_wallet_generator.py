...
# CSRF is mandatory even though generation does not mutate the DB.
...
# Nothing is stored at generation time: backup confirmation comes first.
conn = sqlite3.connect(db_path)
...
conn.close()
check('Solana key is persisted only as encrypted blob', row[0].startswith('ENC:IDENTITY_WALLET:') and payload['solana_private_key'] not in row[0][:len('ENC:IDENTITY_WALLET:')])
check('EVM wallet address is persisted for all EVM chains', row[2].lower() == payload['evm_address'].lower())
check('EVM key is persisted in existing encrypted EVM column', row[3].startswith('ENC:IDENTITY_WALLET:'))
...
# Privacy guard must not become a general key-export bypass.
