...
def test_authenticated_legacy_mutations_fail_closed_without_csrf():
    ...
    paths=("/api/instant-trade","/api/copy-trade/toggle","/api/push/subscribe","/api/push/unsubscribe","/api/settings/save","/api/settings/trading-profile","/api/follow/toggle","/api/invite/respond","/api/post/7/edit","/api/post/7/delete","/api/admin/invite")
...
def test_money_and_social_clients_send_csrf_tokens():
    ...
    assert "_pushCsrfHeaders" in push and "X-CSRF-Token" in push
    assert "_notifCsrfHeaders" in notif and "X-CSRF-Token" in notif
...
def test_high_risk_dom_sinks_escape_or_avoid_untrusted_html():
    ...
    # Stronger than a data-* attribute: build the user-tag rows as DOM nodes
    # and attach click listeners without interpolating usernames into HTML/JS.
    assert "resultBox.replaceChildren()" in js
    assert "row.addEventListener('click',function(){selectUserTag(u.username)})" in js
    ...
    assert "const _safeHeader=safeImageUrl" in js and "const _safeLogo=safeImageUrl" in js
    assert "function safeMediaUrl(value)" in live
    assert "var safeUrl = safeMediaUrl(imgUrl);" in live
    assert "strong.textContent=String(username||'')" in traders
    assert "desc.replaceChildren()" in traders
